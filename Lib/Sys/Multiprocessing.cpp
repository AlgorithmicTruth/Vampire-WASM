...
{
  SYSTEM_FAIL("fork() not available in WebAssembly", ENOSYS);
  return -1;
}
#else
pid_t Multiprocessing::fork()
...
{ resValue = -1; return -1; }
void Multiprocessing::kill(pid_t, int) {}
void Multiprocessing::killNoCheck(pid_t, int) {}
pid_t Multiprocessing::poll_children(bool &exited, bool &signalled, int &code)
{ exited = false; signalled = false; code = -1; return -1; }
#else
pid_t Multiprocessing::waitForChildTermination(int& resValue)
{
