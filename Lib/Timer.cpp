...
/* ---- Emscripten stubs: no timer thread, no perf events ---- */
static std::chrono::time_point<std::chrono::steady_clock> START_TIME;

namespace Lib {
namespace Timer {
void reinitialise(bool) {
...
void printMSString(std::ostream &str, int ms) {
  if(ms<0) { str << '-'; ms = -ms; }
  int sec=ms/1000, msonly=ms%1000;
  str << (sec ? sec : 0) << '.';
  if(msonly<100) { str<<'0'; if(msonly<10) { str<<'0'; if(!msonly) str<<'0'; } }
  str << msonly << " s";
}
std::string msToSecondsString(int ms) {
  return std::to_string(static_cast<float>(ms)/1000)+" s";
}
bool instructionLimitingInPlace() { return false; }
long elapsedMegaInstructions() { return 0; }
